...
def _seed_indicators_once(auth_headers):
    """Seed I-13 / I-44 / I-55 once per module run."""
    from app.main import app
    items = [
        {"indicator_code": "I-13", "name": "预算管理制度",
         "category": "经济活动", "subcategory": "（一）预算业务控制", "max_score": 2},
        {"indicator_code": "I-44", "name": "合同管理制度",
         "category": "经济活动", "subcategory": "（六）合同控制", "max_score": 2},
        {"indicator_code": "I-55", "name": "补充指标",
         "category": "补充", "subcategory": "补充指标", "max_score": 2},
    ]
    with TestClient(app) as c:
        # idempotent: import 接口会跳过已存在的 indicator_code
        files = {"file": ("seed.json",
                          io.BytesIO(json.dumps(items).encode("utf-8")),
                          "application/json")}
        r = c.post("/api/indicators/import", files=files, headers=auth_headers)
